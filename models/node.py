...
class NODE(BaseModelTorch):
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        data = node_lib.Dataset(self.args.dataset, random_state=815,
                                X_train=np.array(X, dtype=float), y_train=np.array(y, dtype=float),
                                X_valid=np.array(X_val, dtype=float), y_valid=np.array(y_val, dtype=float))
    ...
    def predict_helper(self, X):
        X_test = torch.as_tensor(np.array(X, dtype=float), device=self.device, dtype=torch.float)
